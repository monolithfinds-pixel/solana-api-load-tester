...
for idx, key_data in enumerate(wallets):
    # Assign an RPC to this wallet (Wallet 0 uses RPC 0, Wallet 10 uses RPC 0 again)
    ...
    if "error" in air_resp:
        print(f"Airdrop failed: {air_resp['error'].get('message', 'Unknown')}")
        continue
    ...
    # 3. Farm Volume (10 transactions per wallet)
    if balance > 0:
        for i in range(10):
            try:
